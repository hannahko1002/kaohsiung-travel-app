...
if st.session_state["auth_status"] is None:
    ...
    with gate_col2:
        ...
        with login_tab:
            login_user = st.text_input("帳號", key="login_user_input")
...
def _debug_api_key_status():
    """回傳除錯資訊字串，協助排查為什麼讀不到 Gemini API 金鑰。"""
    lines = []

    cwd = os.getcwd()
    lines.append(f"📁 目前程式執行的工作目錄：\n`{cwd}`")

    expected_path = os.path.join(cwd, ".streamlit", "secrets.toml")
    exists = os.path.exists(expected_path)
    lines.append(
        f"🔍 系統預期的 secrets.toml 路徑：\n`{expected_path}`\n"
        f"→ 這個路徑實際{'✅ 找得到檔案' if exists else '❌ 找不到檔案'}"
    )

    try:
        has_key = "GEMINI_API_KEY" in st.secrets
        lines.append(f"🔑 st.secrets 裡面{'✅ 有' if has_key else '❌ 沒有'} GEMINI_API_KEY 這個欄位")
        if has_key:
            val = st.secrets["GEMINI_API_KEY"]
            preview = f"{val[:6]}...{val[-4:]}" if val and len(val) > 12 else "(空值或太短)"
            lines.append(f"👀 讀到的金鑰開頭/結尾（僅供核對，非完整金鑰）：`{preview}`")
    except Exception as e:
        lines.append(f"⚠️ 讀取 st.secrets 時發生錯誤：`{e}`")

    env_val = os.environ.get("GEMINI_API_KEY")
    lines.append(f"🌱 環境變數 GEMINI_API_KEY {'✅ 有設定' if env_val else '❌ 沒有設定'}")
...
if _get_gemini_api_key() is None:
    st.error(
        ...
        "```\nGEMINI_API_KEY = \"AQ.Ab8RN6LSLc7ZU8ELqxDS8XXvZDJw8TWBvZ5w3CblkRhnGRhKHg\"\n```\n\n"
