...
if st.session_state["auth_status"] is None:
    st.markdown('<div class="main-title">高雄 100+ 吃喝玩樂導覽系統</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">登入後即可收藏喜愛的地點，方便下次快速查看！</div>', unsafe_allow_html=True)

    gate_col1, gate_col2, gate_col3 = st.columns([1, 1.4, 1])
    with gate_col2:
        login_tab, register_tab = st.tabs(["🔑 登入", "📝 註冊新帳號"])
        ...
        with login_tab:
            ...
            if st.button("登入", type="primary", use_container_width=True):
                if verify_login(login_user, login_pw):
                    ...
                    st.error("帳號或密碼不正確，請再試一次。")

        with register_tab:
            reg_user = st.text_input("設定帳號", key="reg_user_input")
            reg_pw = st.text_input("設定密碼", type="password", key="reg_pw_input")
        ...
        st.divider()
        st.caption("不想註冊？也可以先逛逛看：")
        if st.button("🚶 以遊客身分繼續瀏覽（無法收藏地點）", use_container_width=True):
...
GEMINI_API_KEY_HARDCODED = "AQ.Ab8RN6KMgHfAWZubAY4Ine2dXayjJl185AWJ81tBkWLQCu9Dsw"
...
def _debug_api_key_status():
    ...
    if has_key:
        ...
        lines.append(
            "請把程式碼最上方 `GEMINI_API_KEY_HARDCODED = \"AQ.Ab8RN6KMgHfAWZubAY4Ine2dXayjJl185AWJ81tBkWLQCu9Dsw\"` "
...
- address：地址（格式須為「高雄市{district}...」，請提供你所知最接近實際的地址）
- hours：營業時間
...
請以繁體中文撰寫，語氣生動、貼近觀光導覽文案風格。
每一個景點請提供以下欄位：
- name：真實景點名稱（不可為範本化名稱）
- type：類型（例如「文創展覽」）
- address：地址（須為「高雄市...」的完整地址，請提供你所知最接近實際的地址）
- hours：開放時間
...
with st.sidebar:
    ...
    if IS_GUEST:
        st.info("🚶 目前為**遊客瀏覽模式**\n\n登入後可收藏地點！")
        if st.button("🔑 前往登入 / 註冊", use_container_width=True):
        ...
        with st.expander(f"⭐ 我的收藏（{len(my_favorites)}）", expanded=False):
            if not my_favorites:
                st.caption("目前還沒有收藏地點，逛逛看並點擊⭐收藏吧！")
            else:
...
if st.session_state.get("current_item"):
    ...
    with btn_col4:
        ...
        if IS_GUEST:
            st.button("🔒 登入才能收藏", type="secondary", use_container_width=True, disabled=True)
        else:
            already_fav = is_favorited(CURRENT_USER, item)
            fav_label = "💔 取消收藏" if already_fav else "⭐ 收藏地點"
...
# 尚未生成或選擇景點時，顯示首頁提示與熱門按鈕
    ...
    st.markdown('<div class="main-title">高雄 AI 即時美食景點導覽系統</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-title">【高雄商圈振興專案】即時生成在地美食與特色景點，精準導流實體人潮！</div>', unsafe_allow_html=True)
