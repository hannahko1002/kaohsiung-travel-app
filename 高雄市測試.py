...
GEMINI_API_KEY_HARDCODED = "AQ.Ab8RN6JCIbJ_b94iwXqJKhgRD3BnBPSj_ilEcSpSwYTYuAX1MA"
...
def _debug_api_key_status():
    ...
    if has_key:
        ...
        lines.append(
            "請把程式碼最上方 `GEMINI_API_KEY_HARDCODED = \"AQ.Ab8RN6JCIbJ_b94iwXqJKhgRD3BnBPSj_ilEcSpSwYTYuAX1MA\"` "
...
def get_parking_info_by_mode(item, mode):
    ...
    elif "公車" in mode or "バス" in mode or "버스" in mode or "Bus" in mode:
        return item.get("parking_bus", "鄰近設有公車站牌，可搭乘公車直達或步行前往。")
    else:
...
if st.session_state.get("current_item"):
    ...
    with col2:
        ...
        transport_mode = st.selectbox(
            t("transport_label"),
            [t("transport_car"), t("transport_scooter"), t("transport_bike"), t("transport_mts"), t("transport_bus")],
            index=0
        )
        ...
        if user_input and isinstance(user_input, str):
            ...
            if any(keyword in user_input for keyword in ["天氣", "氣溫", "溫度", "下雨", "雨", "幾度", "熱嗎", "帶傘", "天候"]):
                ...
                if "YouBike" in transport_mode or "腳踏車" in transport_mode:
                    ...
                    icon = "🚊"
                elif "公車" in transport_mode or "バス" in transport_mode or "버스" in transport_mode or "Bus" in transport_mode:
                    search_label = "公車站"
                    # 搜尋關鍵字：公車站 near 店家地址
                    search_query = f"公車站 near {place_address}"
                    icon = "🚌"
                else:
                    search_label = "停車場"
                    # 搜尋關鍵字：停車場 near 店家地址
